...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="<?php echo e(route('seller.orders.report')); ?>" class="px-4 py-2.5 border border-indigo-200 text-indigo-600 rounded-xl text-sm font-semibold hover:bg-indigo-50 transition-all">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
        <form method="GET" action="<?php echo e(route('seller.orders')); ?>" class="flex flex-col md:flex-row gap-3">
            <div class="flex-1 relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><i class="fas fa-search"></i></span>
                <input type="text" name="q" value="<?php echo e($search); ?>" placeholder="Cari kode pesanan atau nama pembeli..."
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-all">
                        <td class="py-4 px-4">
                            ...
                            <p class="text-gray-400 text-xs"><?php echo e($order->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i')); ?></p>
                        </td>
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold text-xs">
                                    <?php echo e(strtoupper(substr($order->buyer->name, 0, 1))); ?>

                                </div>
                                <div>
                                    <p class="font-medium text-gray-800 text-sm"><?php echo e($order->buyer->name); ?></p>
                                    <p class="text-gray-400 text-xs"><?php echo e($order->buyer->phone); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-4">
                            <p class="font-bold text-gray-800">Rp <?php echo e(number_format($order->total_price, 0, ',', '.')); ?></p>
                            <p class="text-gray-400 text-xs"><?php echo e($order->items->count()); ?> item</p>
                        </td>
                        <td class="py-4 px-4">
                            <span class="px-2.5 py-1 rounded-full text-xs font-semibold <?php echo e($order->payment_status==='paid'?'bg-green-100 text-green-700':'bg-yellow-100 text-yellow-700'); ?>">
                        ...
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-2">
                                <a href="<?php echo e(route('seller.orders.show', $order)); ?>" class="px-3 py-1.5 bg-indigo-100 text-indigo-700 rounded-lg text-xs font-semibold hover:bg-indigo-200 transition-all">
                                    Detail
                                </a>
                                <form method="POST" action="<?php echo e(route('seller.orders.destroy', $order)); ?>" onsubmit="return confirm('Hapus pesanan ini?')">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\TA-SBD-KEL26-main\TA-SBD-KEL26-main\resources\views/seller/orders/index.blade.php ENDPATH**/ ?>